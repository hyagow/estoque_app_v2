...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if user and user['password'] == form_data.password:
        return {"access_token": f"{user['role']}:{form_data.username}", "token_type": "bearer"}
...
async def create_produto(produto: Produto, token: str = Depends(oauth2_scheme)):
    ...
    with get_db_connection() as conn:
        conn.execute(
            ...
            (produto.nome, produto.categoria, produto.quantidade, produto.preco, produto.localizacao, produto.nota_fiscal)
        )
        conn.commit()
...
async def update_produto(produto_id: int, produto: Produto, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "estoquista":
        raise HTTPException(status_code=403, detail="Acesso negado.")
    ...
    with get_db_connection() as conn:
        conn.execute(
            ...
            (produto.nome, produto.categoria, produto.quantidade, produto.preco, produto.localizacao, produto.nota_fiscal, produto_id)
        )
        conn.commit()
...
async def create_movimentacao(movimentacao: Movimentacao, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "estoquista":
        raise HTTPException(status_code=403, detail="Acesso negado.")
    ...
    with get_db_connection() as conn:
        conn.execute(
            ...
            (movimentacao.produto_id, movimentacao.tipo, movimentacao.quantidade, datetime.now().isoformat(), movimentacao.pedido_id, movimentacao.observacao)
        )
        conn.commit()
        return {"mensagem": "Movimentação cadastrada com sucesso!"}

@app.post("/compras/solicitar/", response_model=dict)
async def solicitar_compra(solicitacao: SolicitarCompra, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "usuario":
        raise HTTPException(status_code=403, detail="Acesso negado.")
...
async def autorizar_compra(solicitacao: AutorizarCompra, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "gerente":
        raise HTTPException(status_code=403, detail="Acesso negado.")
...
async def relatorio_semanal(token: str = Depends(oauth2_scheme)):
    ...
    with get_db_connection() as conn:
        ...
        for mov in movimentacoes:
            produto = conn.execute("SELECT nome, quantidade FROM produtos WHERE id = ?", (mov['produto_id'],)).fetchone()
            if produto:
                relatorio.append({
                    ...
                    "quantidade_final": produto['quantidade'] + mov['total_entrada'] - mov['total_saida']
                })
    return relatorio

@app.post("/produtos/entrada/", response_model=dict)
async def registrar_entrada(produto_id: int, quantidade: int, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "estoquista":
        raise HTTPException(status_code=403, detail="Acesso negado.")
...
async def registrar_saida(produto_id: int, quantidade: int, token: str = Depends(oauth2_scheme)):
    role = get_user_role(token)
    if role != "estoquista":
        raise HTTPException(status_code=403, detail="Acesso negado.")
